"""
Iris - customer-support agent (LAYER 1, network controls) — AG-UI server.

IDENTICAL agent code to the baseline (full SQL via run_query + open http_request),
served over AG-UI. The ONLY thing that changes at Layer 1 is the deployment: the
runtime runs in VPC mode and the VPC's egress is filtered by a Route 53 DNS
Firewall that allows ONLY the sanctioned Shipment Service host and denies every
other domain (incl. the attacker's exfil URL). So the same http_request tool that
exfiltrated in baseline now reaches the shipment service but is blocked for the
attacker URL — same agent, different network perimeter.

AgentCore Runtime is deployed with serverProtocol=AGUI and proxies requests to
this container:
...
def _build_agui_agent(model_id: str | None):
    ...
    return StrandsAgent(
        agent=strands_agent,
        name="iris_layer1",
        description="Iris Layer 1 (VPC + DNS Firewall) customer-support agent",
    )
