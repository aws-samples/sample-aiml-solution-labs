...
export default function ChatPanel({ sessionId, userId, messages: initialMessages, onFirstMessage, getCredentials, isAdmin }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "", timestamp: getETTimestamp(), streaming: true }]);

      await awsService.chatStream(creds, {
        prompt, sessionId, userId,
        sessionTitle: isFirst ? prompt.slice(0, 80) : undefined,
        modelId: selectedModel.value || undefined,
      }, (accumulatedText) => {
        // Update the last message (the streaming assistant message) with accumulated text
        setMessages((prev) => {
          const updated = [...prev];
