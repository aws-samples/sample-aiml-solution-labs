...
const AGENT_NAME = "GenAiTcoRoiTool_AnalystAgent";
...
class AwsService {
  ...
  /**
   * Send a chat message with streaming — calls onChunk(accumulatedText) as tokens arrive.
   */
  async chatStream(credentials, { prompt, sessionId, userId, sessionTitle, modelId }, onChunk) {
    // Save user message
    if (userId && sessionId) {
      await this.saveMessage(credentials, userId, sessionId, sessionTitle, "user", prompt, modelId);
    }
    ...
    // Save final response
    if (userId && sessionId && accumulated) {
      await this.saveMessage(credentials, userId, sessionId, sessionTitle, "assistant", accumulated, modelId);
    }
