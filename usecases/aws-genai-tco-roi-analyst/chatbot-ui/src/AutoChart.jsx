...
export function tryRenderChart(parsed) {
  ...
  // Handle array of chart objects — render each as a separate chart
  if (Array.isArray(parsed)) {
    const chartItems = parsed.filter(
      (item) => item && typeof item === "object" && (item.chart_type || item.chart_data)
    );
    if (chartItems.length > 0) {
      const rendered = chartItems.map((item, idx) => tryRenderSingleChart(item, idx));
  ...
  // Check if a nested field (e.g., "charts") contains an array of chart objects
  for (const val of Object.values(parsed)) {
    if (Array.isArray(val) && val.length > 0 && val[0] && val[0].chart_type) {
      const rendered = val.map((item, idx) => tryRenderSingleChart(item, idx));
      const valid = rendered.filter(Boolean);
  ...
  // Single chart object
  return tryRenderSingleChart(parsed, 0);
}

function tryRenderSingleChart(parsed, key) {
  if (!parsed || typeof parsed !== "object") return null;
  ...
  return (
    <div key={`chart-${key}`} style={{ margin: "12px 0" }}>
...
function findChartData(obj) {
  ...
  if (obj && typeof obj === "object") {
    if (obj.chart_data) {
      // Handle chart_data as array
      if (Array.isArray(obj.chart_data)) return normalizeNames(obj.chart_data);
      // Handle chart_data as stringified JSON
      if (typeof obj.chart_data === "string") {
        try {
          const parsed = JSON.parse(obj.chart_data);
          if (Array.isArray(parsed)) return normalizeNames(parsed);
