...
Reads the AWS pricing index and fetches pricing documents for Bedrock-related services.
...
Usage:
    ...
    python price_doc_scraper.py --region us-east-1
    python price_doc_scraper.py --output ./pricing_data

By default, only the 4 Bedrock-related services are scraped:
AmazonBedrock, AmazonBedrockAgentCore, AmazonBedrockService, AmazonBedrockFoundationModels.
Use --service to override with specific services.
ALL regions are scraped by default. Use --region to filter to specific regions.
"""
...
# Reference list of common US regions (used only when --region is explicitly passed)
DEFAULT_REGIONS = ['us-west-2', 'us-east-1', 'us-gov-west-1', 'us-gov-east-1']

# Default services to scrape when no --service flag is passed
DEFAULT_SERVICES = [
    'AmazonBedrock',
...
def get_pricing_documents(
    ...
    Args:
        ...
        regions_filter: Optional list of region codes to include (None = all regions)
...
def main():
    ...
    parser.add_argument('--service', type=str, action='append',
                        help='Service to fetch (can specify multiple). '
                             'Default: AmazonBedrock, AmazonBedrockAgentCore, '
                             'AmazonBedrockService, AmazonBedrockFoundationModels')
    ...
    parser.add_argument('--region', type=str, action='append',
                        help='Filter to specific region(s) (default: all regions). Can specify multiple times.')
    parser.add_argument('--all-regions', action='store_true',
                        help='(No-op, kept for backward compatibility) All regions are scraped by default.')
    ...
    # Default: scrape ALL regions (regions_filter=None)
    # --region flag: filter to only the specified region(s)
    # --all-regions: no-op (all regions is already the default)
    regions = args.region  # None when not specified = all regions
    services = args.service if args.service else DEFAULT_SERVICES
