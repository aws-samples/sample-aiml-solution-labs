...
models across specified regions. Saves one text file per quota entry into
quota_data/{region}/{sanitized_quota_name}.txt

Usage:
    python quota_doc_scraper.py
    python quota_doc_scraper.py --region us-east-1 --region eu-west-1
    python quota_doc_scraper.py --all-regions
    python quota_doc_scraper.py --list
"""
...
def main():
    ...
    parser.add_argument('--all-regions', action='store_true',
                        help='Scrape all AWS regions')
    parser.add_argument('--list', action='store_true',
                        help='List default regions and exit')
    args = parser.parse_args()

    if args.list:
        print(f"Default regions ({len(DEFAULT_REGIONS)}):")
